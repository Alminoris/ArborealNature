...
public class CustomSpreadableBlock extends SpreadableBlock implements Fertilizable
{
    public static final MapCodec<CustomSpreadableBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            createSettingsCodec(),
            Registries.BLOCK.getCodec().fieldOf("block").forGetter(decorator -> decorator.dirtReplacable)
    ).apply(instance, CustomSpreadableBlock::new));

    private final Block dirtReplacable;

    @Override
    public MapCodec<CustomSpreadableBlock> getCodec() {
        return CODEC;
    }

    public CustomSpreadableBlock(AbstractBlock.Settings settings, Block dirtReplacable)
    {
        super(settings);
        this.dirtReplacable = dirtReplacable;
    }
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random)
    {
        if (!canSurvive(state, world, pos)) {
            world.setBlockState(pos, dirtReplacable.getDefaultState());
        } else {
            if (world.getLightLevel(pos.up()) >= 9) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (world.getBlockState(blockPos).isOf(dirtReplacable) && canSpread(blockState, world, blockPos)) {
