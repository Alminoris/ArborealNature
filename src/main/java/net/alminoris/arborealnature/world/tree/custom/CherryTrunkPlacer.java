...
{
    private static final Codec<UniformIntProvider> BRANCH_START_OFFSET_FROM_TOP_CODEC = UniformIntProvider.CODEC.flatXmap(
            ...
            instance -> fillTrunkPlacerFields(instance)
                    .<IntProvider, IntProvider, UniformIntProvider, IntProvider>and(
                            instance.group(
                                    ...
                                    BRANCH_START_OFFSET_FROM_TOP_CODEC.fieldOf("branch_start_offset_from_top").forGetter(trunkPlacer -> trunkPlacer.branchStartOffsetFromTop),
    ...
    public CherryTrunkPlacer(int baseHeight, int firstRandomHeight, int secondRandomHeight, IntProvider branchCount,
            IntProvider branchHorizontalLength, UniformIntProvider branchStartOffsetFromTop, IntProvider branchEndOffsetFromTop)
    {
                                                 ...
                                                 Random random, int height, BlockPos startPos, TreeFeatureConfig config)
    {
        ...
        Function<BlockState, BlockState> function = state ->
        {
            if (state.contains(PillarBlock.AXIS))
                return state.with(PillarBlock.AXIS, direction.getAxis());
            return state;
        };
    ...
    private FoliagePlacer.TreeNode generateBranch(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos,
            TreeFeatureConfig config, Function<BlockState, BlockState> withAxisFunction, Direction direction, int branchStartOffset, boolean branchBelowHeight,
