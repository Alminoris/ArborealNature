...
public class ExtendedSaplingGenerator extends SaplingGenerator {
    ...
    // Convenience constructor for regular + large 2x2 trees
    public ExtendedSaplingGenerator(String id,
                                    RegistryKey<ConfiguredFeature<?, ?>> regularFeature,
                                    RegistryKey<ConfiguredFeature<?, ?>> large2x2Feature) {
        this(id, 0.0f,
                Optional.ofNullable(large2x2Feature), Optional.empty(), Optional.empty(), Optional.empty(),
                Optional.empty(),
                Optional.ofNullable(regularFeature), Optional.empty(), Optional.empty(), Optional.empty());
    }

    // Full constructor for all variants
    ...
    /* ===============================
       Helpers for NxN detection & placement
       =============================== */
    ...
    public boolean generate(ServerWorld world, ChunkGenerator chunkGenerator, BlockPos pos, BlockState state, Random random) {
        int[] sizes = {5, 4, 3, 2}; // largest first
        for (int size : sizes) {
            ...
            int half = size / 2;
            for (int offsetX = -half; offsetX <= 0; offsetX++) {
                for (int offsetZ = -half; offsetZ <= 0; offsetZ++) {
        ...
        // Fallback to small tree
        RegistryKey<ConfiguredFeature<?, ?>> smallTree = getSmallTreeFeature(random, areFlowersNearby(world, pos));
        ...
        if (entry.value().generate(world, chunkGenerator, random, pos)) {
            if (world.getBlockState(pos).equals(fluidState)) {
    ...
    private RegistryKey<ConfiguredFeature<?, ?>> getSmallTreeFeature(Random random, boolean flowersNearby) {
        if (random.nextFloat() < this.rareChance) {
            if (flowersNearby && this.rareBeesVariant.isPresent()) return this.rareBeesVariant.get();
            if (this.rareRegularVariant.isPresent()) return this.rareRegularVariant.get();
            if (this.rareMegaVariant.isPresent()) return this.rareMegaVariant.get();
        }

        if (flowersNearby && this.beesVariant.isPresent()) return this.beesVariant.get();
    ...
    private boolean areFlowersNearby(WorldAccess world, BlockPos pos) {
        for (BlockPos check : BlockPos.Mutable.iterate(pos.down().north(2).west(2), pos.up().south(2).east(2))) {
            if (world.getBlockState(check).isIn(BlockTags.FLOWERS)) return true;
        }
        return false;
    }
    ...
    protected RegistryKey<ConfiguredFeature<?, ?>> getTreeFeature(Random random, boolean bees) {
        return getSmallTreeFeature(random, bees);
    }
}
