                ...
                if (random.nextFloat() < probability) {
                    ...
                    if (providerState.isOf(ModBlocks.FLAT_GRASS)) {
                        if (world.testBlockState(carpetImmutable, BlockState::isAir) &&
                                world.testBlockState(groundImmutable, state -> validBlocksWithoutWater.contains(state.getBlock()))) {
                            generator.replace(carpetImmutable, providerState);
                            // replace ground with coarse dirt — groundImmutable is safe (we checked it's not a log)
                            generator.replace(groundImmutable, Blocks.COARSE_DIRT.getDefaultState());
                        }
                    } else {
                        if (world.testBlockState(carpetImmutable, BlockState::isAir) &&
